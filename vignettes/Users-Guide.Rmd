...
This user's guide includes R-language scripts that are used to cluster the Colorado data. We highly recommend that you copy these scripts into a file and execute the scripts. This effort will help you become familar with the processing and its analysis. Some details of your processing results will differ from the results presented in this user's guide because of the random nature of the calculations. Nonetheless, the general features of your processing results will be the same.

The remainder of the user's guide is divided into 8 sections: [Organization of the geochemical data], [Preparatory steps], [Data transformation and analysis], [Sampling the posterior probability density function], [Analysis and processing of the Monte Carlo samples], [Cluster visualization], [Splitting the geochemical data], [Processing the splits], and [References].

# Organization of the geochemical data

## First element

The geochemical data is organized as a R-language list with three elements. The first list element is called "concData", and it contains the geochemical concentrations and related information. The list element is a data container, which is called a `SpatialPointsDataFrame`. This container is defined in the "sp" package and was designed specifically for spatial data. A SpatialPointsDataFrame comprises several sub-containers, which are fully described in the sp package documentation. Two sub-containers are especially important here. 

The first sub-container is an R-language data frame that stores the geochemical concentrations for each field sample. Each row of this data frame pertains to one field sample, and the name of the row is the name of the field sample. Each column pertains to one element, and the name of the column is the abbreviated name of the element (for example, "Al" or "Zn"). So, a row of the data frame comprises all element concentrations for one field sample, and a column of the data frame comprises the concentrations of one element for all field samples. 
...
## Third element
The third list element is called "constSumValue". Geochemical data sum to a constant value even if some element concentrations are not reported. For geochemical data, the constant sum value typically is 1000000 mg/kg, so the `constSumValue` equals 1000000.
...
The geochemical data are transformed first with the isometric log-ratio transform and second with the principal components transform. Both operations are performed by function `transformGcData`:
```
...
```
The principal components and other related variables are stored in container `transData`. These variables are needed for the clustering.

Function `transformGcData` has a second argument `alpha`, which is assigned a default value in the above scripts. Argument `alpha` is the fraction of the data that is used to calculate the mean vector and the covariance matrix; it may range from 0.50 to 1.00. Relative low values, near 0.50, are appropriate for data with high measurement error; whereas relatively high values, near 1.00, are appropriate for data with low measurement error. There is no formula for selecting an appropriate value---it is a matter of judgement. Our quality control analyses suggest that the geochemical concentrations in `CoGeochemData`, and hence in `gcData`, have relatively low measurement error. Consequently, `alpha` is chosen to be 0.98, which happens to be its default value. 
...
The principal components are now analyzed graphically to check that they are properly computed and to select appropriate parameters for the clustering. Check the distributions of the principal components:
```
# plot boxplots and violinplots of the principal components
plotEdaDist(transData)   
```

![Figure 2. (upper) Boxplots and (lower) violinplots of the principal components.](figures/EdaDist.png)

The boxplots and the violinplots (Figure 4) show that the distributions for each component are centered at zero and that the spread of the distributions decreases as the component number increases. This behavior is expected for principal components. For these data, the distributions for each component are unimodal.
...
![Figure 3. (upper) Correlation matrix for the principal components. The red diagonal corresponds to the diagonal of the correlation matrix. (lower) Histogram of the correlations.](figures/EdaCorr.png)
...
Perform Monte Carlo sampling of the posterior probability density function (pdf):
```
...
```
Function argument `nCpuCores` specifies that 7 central processing unit (cpu) cores will be requested from the operating system. Of course, the number of processors that you request must be less than or equal to the number on your computer. Function argument `nChainsPerCore` specifies that 5 separate sampling chains will be computed by each core. So, a total of 35 chains are computed. Our experience is that many chains are needed because there may be multiple modes, and that approximately 35 chains is adequate.
...
You may notice switching between chains: For example, theta could be 0.42 for chain 1 and 0.58 for chain 2. Associated with this switch in theta are corresponding switches for the other 4 model parameters. Such switching is common and normal. 
...
Both difficulties are overcome by standardizing the compositional means, which involves centering the compositional means and then scaling them (Pawlowsky-Glahn and others, 2105, p. 68-69). The parameters for centering and scaling are computed with function `calcSimplexStats`:
```
simplexStats <- calcSimplexStats(gcData)
save(simplexStats, file = "SimplexStats.dat")
```
Function `plotStdCompMeans` uses these parameters to plot the standardized compositonal mean:
```
plotStdCompMeans( simplexModPar, simplexStats, gcData, elementOrder)
```

![Figure 10. Standardized compositional means for the two pdfs. A dot represents the median of the Monte Carlo samples, and vertical line represents the 95% credible interval. The horizontal black line represents the barycenter.](figures/StdCompMeans.png)

For each chemical element, the median and the 95% credible interval summarize the distribution of the standardized compositional mean. The percentage for the credible interval can be changed using the argument `intervalPercentage`.

This plot readily shows how the standardized element concentrations change between pdfs 1 and 2. For example, Figure 10 shows that element indium (In) is much higher in pdf 2 than it is in pdf 1. This information is combined with the spatial distribution of the clusters (which is discussed in [Mapping the clusters]), greatly facilitating the interpretation of the geochemical data. 

This plot depends upon the parameters that are used for the centering and scaling. Consequently, this plot can be compared only to other plots for which the same parameters are used. Because this processing is the first clustering of the Colorado data, there are no other plots. However, for subsequent clustering, there are such plots. This important issue is discussed further in section [Processing the clusters].
...
The range of the variances in the matrix may be large, so the variances are scaled by the square root function. Consequently, the pixels in the Figure 11 represent standard deviations of the log-ratios between the respective chemical elements. Again, the order of the elements is specified by vector `element order`.


## Mapping the clusters

The conditional probabilities that the field samples are associated with pdf 1 was calculated in section [Checking the model]. A map showing showing the spatial distribution of these associations is very helpful when interpreting the geochemical data. To this end, the conditional probabilities must be grouped into meaning categories. 

We have found that 4 categories are useful for interpetation. The first category indicates a strong association with pdf 1 and comprises those field samples for which the median conditional probabilities range from 0.9 to 1.0. The second category indicates a moderate association with pdf 1 and comprises those field samples for which the median conditional probabilities range from 0.5 to 0.9. The third category indicates a moderate association with pdf 2 (namely, a weak association with pdf 1) and comprises those field samples for which the median conditional probabilities range from 0.10 to 0.5. The fourth category indicates a strong association with pdf 2 (namely, a very weak association with pdf 1) and comprises those field samples for which the median conditional probabilities range from 0.0 to 0.1. These categories define clusters of the field samples, which can be plotted:
```
map('state', regions = 'colorado', fill = TRUE, col = "gray60", border = "white")
map.axes()
...
![Figure 12. Map of the clusters. Each dot represents the location of a field sample. Dot color represents the strength of the association between a field sample and a pdf in the finite mixture model: Blue and green indicate strong and moderate assocation, respectively, with pdf 1; red and yellow indicate strong and moderate association, respectively, with pdf 2.](figures/ClusterMap.png)

The number of categories and the related intervals of the median conditional probability are specified in argument `probIntervals` of function `plotClusters`. Likewise, the plot symbols, their sizes, and their colors may also be changed. However, we found that the default values for these arguments work well for many maps.

The value of a cluster map becomes apparent when it is viewed with the standardized compositional means (Figure 10) and the scaled variation matrices (Figure 11). For example, the cluster of blue dots, which has a strong association with pdf 1, has higher concentrations of potassium (K), rubidium (Rb), barium (Ba), and arsenic (As) than the cluster of red dots does. In addition, the cluster of blue dots has lower variability in the log-ratios associated with K, Rb, Ba, and As than the cluster of red dots does. This information is then interpreted by relating it to independent information, such as geologic information.

# Splitting the geochemical data

The field samples in each cluster may processed to find finer-scale clusters. To this end, the field samples must be split, and this operation is accomplished with function `splitGcData`: 
```
...
```
Observe that both list elements are renamed `gcData` so that the R-language scripts, that are used for processing, do not need editting. 

# Processing the splits

The geochemical data in "Process11" and "Process12" are clustered by applying  the processing that already has been described in this user's guide. There are only two issues that require further discussion. 

First, in section [Plotting the compositional mean], the plot of the standardized compositional mean was created using standardization statistics that were calculated from the geochemical data that were being processed (namely, the geochemical data for the entire survey area). When the plot of the standardized compositional mean is created for, say, "Process11", the standardization statistics likewise could be calculated from the data in "Process11". Although such a plot is invaluable for interpretation, the resulting plot cannot be compared to corresponding plots for other clusters. To overcome this problem, additional plots of the standardized compositional mean may be created using standardization statistics that were generated within "Process1"; these plots can be compared to one another.

The second issue is deciding when to stop the clustering. Heretofore, our experience has been that when the number of field samples in a cluster is about 250 or 300, then the Monte Carlo sampling does not converge. Consequently, we have stopped. As we gain more experience, then this criterion probably be replaced with better criteria.
