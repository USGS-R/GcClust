...
During the finite mixture modeling, two multivariate normal distributions are fit to the principal components. That is, for one distribution, a mean and a standard deviation are estimated for each component, and a correlation matrix is estimated for all components. For the other distribution, the same statistics are calculated. 

Consider the violinplot plot for component 1. The highest densities range from approximately -2.5 to 2.5, so the two means for this component are probably within this range. The two standard deviation associated with these two means are much harder to estimate. Nonetheless, it is possible to estimate a very rough upper bound: Whatever the value of the two means, the standard deviations will be smaller than the entire range of the data, which is about 9.2. In our experience, the standard deviations are usually less then half the range of the data, which would be about 4.6.

# Sampling of the posterior probability density function
The sampling of the posterior probability density function is performed with the following code.
```
samplePars <- sampleFmm(transData, nPCs,
                        nSamplesPerChain = 1000,
                        nChainsPerCore = 5, nCpuCores = 7, procDir = ".")

save(samplePars, file = "SamplePars.dat")
```
Function argument `nCpuCores` specifies that 7 processor (central processing units) cores will be requested from the operating system. Of course, the number of processor that you request must be less than or equal to the number on your computer. Function argument `nChainsPerCore` specifies that 5 separate sampling chains will be computed by each core. So, a total of 35 chains are computed. 

The chains are not returned by function `sampleFmm` because they would require too much memory. Instead, the chains are written to the computer disk in the directory specified by argument `procDir`. The file names have the format 
